...
def getDescription(image):
    ...
    with open(os.path.join(basedir, 'config.json')) as json_file:
...
def getTag(image2):
    ...
    endpoint = config["endpoint"]
    computervision_client = ComputerVisionClient(endpoint, CognitiveServicesCredentials(subscription_key))
    tags_result_local = computervision_client.tag_image_in_stream(image2)
    list_tags=[]
    if (len(tags_result_local.tags) == 0):
        return list_tags
    else:
        for tag in tags_result_local.tags:
            if (tag.confidence *100) >98:
...
def upload():
    if request.method == 'POST' and 'file' in request.files:
        ...
        image_url = os.path.join(current_app.config['ALBUMY_UPLOAD_PATH'], filename)
        image = open(image_url, "rb")
        image2=open(image_url, "rb")
        
        # use this description for alternative text
        alt_text = getDescription(image)
        tags_list=[]
        tags_list = getTag(image2)
        photo = Photo(
            filename=filename,
        ...
        for tag in tags_list:
            results=db.session.query(Tag).filter(Tag.name==tag).all()
            ...
            photo.tags.append(tag_internal)
...
def show_photo(photo_id):
    ...
    return render_template('main/photo.html', photo=photo, comment_form=comment_form,
                           ...
                           pagination=pagination, comments=comments,alt_text=alt_text)
